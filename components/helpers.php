...
function getSessionUser(): array
{
    if (session_status() !== PHP_SESSION_ACTIVE) {
        ...
        else if (defined('FE_SESSION_NAME'))
...
/**
 * Generate a padded SVG favicon with the primary color background
 */
function getFaviconSvg(): string
{
    ...
    $primary = defined('COLOR_PRIMARY') ? COLOR_PRIMARY : '#0B3D91';
    $radius = '20%'; // Adjust for more/less rounding

    // 1. Clean up the SVG: Remove existing width/height so viewBox takes over
    $svg = preg_replace('/(width|height)="\d+"/', '', $svg);

    // 2. Extract or create a ViewBox
    if (preg_match('/viewBox\s*=\s*"([\d\.\-\s]+)"/i', $svg, $m)) {
        ...
        $w = 100;
        $h = 100;
        $svg = str_replace('<svg', '<svg viewBox="0 0 100 100"', $svg);
    }

    // 3. Apply 15% padding logic
    $padX = $w * 0.15;
    $padY = $h * 0.15;
    $newX = $x - $padX;
    $newY = $y - $padY;
    ...
    // 4. Create the Overlay and Mask
    // Using a unique ID to avoid conflicts if multiple SVGs are on one page
    $maskId = 'mask-' . uniqid();

    $overlay = "<defs>
        <clipPath id='$maskId'>
            <rect x='$newX' y='$newY' width='$newW' height='$newH' rx='$radius' />
        </clipPath>
    </defs>
    <rect x='$newX' y='$newY' width='$newW' height='$newH' fill='$primary' clip-path='url(#$maskId)' />";

    // 5. Re-assemble
    // Update the viewBox to the padded version
    $svg = preg_replace('/viewBox\s*=\s*"[^"]*"/i', 'viewBox="' . "$newX $newY $newW $newH" . '"', $svg);

    // Insert the background and start the clipping group
    $svg = preg_replace('/(<svg[^>]*>)/i', '$1' . $overlay . '<g clip-path="url(#' . $maskId . ')">', $svg, 1);

    // Close the group before the closing svg tag
    $svg = str_replace('</svg>', '</g></svg>', $svg);
