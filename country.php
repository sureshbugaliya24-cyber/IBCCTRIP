...
if (empty($resp['success'])) {
    redirect(FRONTEND_URL . '/country.php');
}
...
<div class="relative h-[60vh] min-h-[400px] overflow-hidden">
  ...
  <div class="absolute inset-0 flex flex-col justify-end pb-12 px-4 max-w-7xl mx-auto left-0 right-0">
    ...
    <h1 class="text-4xl md:text-6xl font-black text-white mt-4 flex items-center gap-4">
      <?= e($country['name'] ?? '') ?>
      <span class="text-4xl"><?= e($country['flag_icon'] ?? '') ?></span>
    </h1>
  </div>
</div>
  ...
  <div class="flex flex-col lg:flex-row gap-16">
    ...
    <div class="flex-1 min-w-0">
      <div class="prose max-w-none text-gray-600 leading-relaxed mb-16">
        <h2 class="text-2xl font-extrabold text-gray-900 mb-6">About <?= e($country['name'] ?? '') ?></h2>
      ...
      <div class="mb-20">
        <div class="flex items-center justify-between mb-8">
          ...
          <a href="<?= FRONTEND_URL ?>/state.php" class="text-primary font-bold text-sm hover:underline">View All</a>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <?php foreach ($states as $state): renderStateCard($state); endforeach; ?>
        </div>
      </div>
      <?php endif; ?>
    ...
    <aside class="lg:w-80 shrink-0">
      ...
      <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-8">
        ...
        <div class="space-y-6">
          <?php foreach ($blogs as $blog): renderBlogCard($blog, 'normal'); endforeach; ?>
        </div>
      </div>
      <?php endif; ?>
      ...
      <div class="bg-secondary rounded-3xl p-8 text-white text-center shadow-2xl shadow-secondary/10">
        ...
        <p class="text-white/80 text-sm mb-6">Let our experts design the perfect itinerary for your trip to <?= e($country['name'] ?? '') ?>.</p>
        <a href="<?= FRONTEND_URL ?>/contact.php" class="block bg-white text-secondary font-extrabold py-3.5 rounded-xl hover:bg-gray-50 transition-all">Get Free Quote</a>
      </div>
    </aside>
